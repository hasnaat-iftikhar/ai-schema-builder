...
function getRelationshipsForSQLAlchemy(tableId: string, tables: TableData[], relationships: Relationship[]) {
  try {
    ...
    relationships.forEach((relationship) => {
      ...
      const sourceTable = tables.find((t) => t.id === relationship.source)
      const targetTable = tables.find((t) => t.id === relationship.target)

      if (!sourceTable || !targetTable) return

      if (relationship.source === tableId) {
        if (relationship.type === "one-to-many") {
          ...
          const tableName =
            relationship.through || `${sourceTable.name.toLowerCase()}_${targetTable.name.toLowerCase()}`
          result.push(
      ...
      } else if (relationship.target === tableId) {
        if (relationship.type === "one-to-many") {
          result.push(
