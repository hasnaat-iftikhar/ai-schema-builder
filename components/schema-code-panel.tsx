...
export function SchemaCodePanel({
  ...
  const [code, setCode] = useState<string>(externalCode || "")
  ...
  useEffect(() => {
    // Always generate code when tables or relationships change
  ...
  const toggleEditing = () => {
    if (isEditing) {
      ...
      try {
        ...
        // const { parsedTables, parsedRelationships } = parseCodeToSchema(code, selectedFormat)
        // if (parsedTables.length > 0 && onTablesChange) {
        //   onTablesChange(parsedTables)
        // }
        // if (parsedRelationships.length > 0 && onRelationshipsChange) {
...
const ${table.name.toLowerCase()}Schema = new Schema({
  ${table.columns
    ...
    unique: true
      ...
      if (foreignRel) {
        if (
          (foreignRel.source === table.id && foreignRel.sourceKey === col.name && foreignRel.type === "many-to-one") ||
    ...
    ref: '${
      foreignRel.source === table.id
        ? tables.find((t) => t.id === foreignRel.target)?.name
        : tables.find((t) => t.id === foreignRel.source)?.name
    }'
  }`
        }
      }
...
function getRelationshipsForMongoose(tableId: string, tables: TableData[], relationships: Relationship[]) {
  ...
  // Here we'll add some virtual fields for convenience
  relationships.forEach((rel) => {
    const sourceTable = tables.find((t) => t.id === rel.source)
    const targetTable = tables.find((t) => t.id === rel.target)
